...
class DocBlock {
	...
	<body>
		...
		// Save file
		if (!is_dir(DIR_TO_SCAN."/phpdocfill-resources")) { mkdir(DIR_TO_SCAN."/phpdocfill-resources",0705); chmod(DIR_TO_SCAN."/phpdocfill-resources",0705); }
		if (!is_dir(DIR_TO_SCAN."/phpdocfill-resources/images")) { mkdir(DIR_TO_SCAN."/phpdocfill-resources/images",0705); chmod(DIR_TO_SCAN."/phpdocfill-resources/images",0705); }
		if (isDirEmpty(DIR_TO_SCAN."/phpdocfill-resources/images")) {
			$listOfFiles = RecursiveScanDir(__DIR__."/phpdocfill-resources",'','all');
			foreach ($listOfFiles as $key => $_file) {
				if (!is_file(DIR_TO_SCAN."/phpdocfill-resources/".$_file)) { file_put_contents(DIR_TO_SCAN."/phpdocfill-resources/".$_file, file_get_contents(__DIR__."/phpdocfill-resources/".$_file)); }
			}
		}
		if (!file_put_contents(DIR_TO_SCAN."/PHPDocFill.html", ($out))) die("The file could not be saved. You might not have the proper writing permissions in the ".getcwd()." folder.");
		
		die("The manual has been generated and saved as PHPDocFill.html.<br /><a href='".URL_OF_DIR_TO_SCAN."/PHPDocFill.html' onclick='window.open(this.href);return false'>Click here to view this file in your browser</a>");
	}
}
...
*
* @param string $dir Dir (supposed to exist)
*
* @return bool
*/
...
/**
 * Performs a recursive directory scan to find files
 */
function RecursiveScanDir($dir, $prefix = '', $ext = 'php') {
	$dir = rtrim($dir, '\\/');
	$result = array();
	foreach (scandir($dir) as $f) {
		if ($f[0] !== '.') {
			if (is_dir("$dir/$f")) {
				...
				if ( $ext == 'php' && strtolower(substr(pathinfo($f,PATHINFO_EXTENSION),0,3)) == "php" && pathinfo(__FILE__,PATHINFO_BASENAME ) != pathinfo($f,PATHINFO_BASENAME ) )
					$result[] = $prefix.$f;
				elseif ($ext == 'all' && pathinfo($f,PATHINFO_EXTENSION) != '')
					$result[] = $prefix.$f;
			}
		}
	}
...
if (REQUIRES_PASSWORD && ( (isset($_SERVER['PHP_AUTH_USER']) && $_SERVER['PHP_AUTH_USER'] != APP_USER) || (isset($_SERVER['PHP_AUTH_PW']) && $_SERVER['PHP_AUTH_PW'] != APP_PASSWORD) )) {
...
<br />&nbsp;*/</span>
				...
				<br />
				<b>To <span style='color:#1365ad'>get started</span>, select a file from the menu on the left.</b>
				<br /> 
				<!-- If you don't see any item in the list, try placing PHPDocFill in a folder full of .php files. -->
				<p>If you don't see any item in the list, fill the informations about your project in the next form :</p>
				
				<form action="" method="post">
					<label for="name">Enter the name of your project </label>
					<input type="text" name="name" id="name" value="<?php echo (isset($_SESSION['project']['name']) ? $_SESSION['project']['name'] : '') ?>" size="40" maxlength="255"/><br/>
					<label for="dir">Enter your project directory</label>
					<input type="text" name="dir" id="dir" value="<?php echo (isset($_SESSION['project']['dir']) ? $_SESSION['project']['dir'] : '') ?>" size="40" maxlength="255"/><br/>
					<label for="url">Enter your project url</label>
					<input type="text" name="url" id="url" value="<?php echo (isset($_SESSION['project']['url']) ? $_SESSION['project']['url'] : 'http://') ?>" size="40" maxlength="255"/><br/>
					<button>Save</button>
				</form>
			</div>
		</div>
